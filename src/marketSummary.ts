...
const messages = {
  ...
  fxUp: (r: number) =>
    `기준 환율은 USD/KRW 약 ${r.toLocaleString("ko-KR")}원으로 오름세(달러 강세)입니다.\n미국 주식 투자 시 환차익이 더해져 원화 환산 수익률이 한층 더 높아지는 효과를 기대할 수 있습니다.`,

  fxDown: (r: number) =>
    `기준 환율은 USD/KRW 약 ${r.toLocaleString("ko-KR")}원으로 내림세(원화 강세)입니다.\n미국 주식에서 달러로 수익이 나더라도, 원화로 환산할 때 환율 하락분만큼 수익이 일부 줄어들 수 있으니 참고해 주세요.`,

  fxFlat: (r: number) =>
    `기준 환율은 USD/KRW 약 ${r.toLocaleString("ko-KR")}원으로 큰 변동 없이 안정적인 흐름입니다.\n현재는 환율 효과보다는 개별 종목의 주가 등락이 미국 주식 원화 수익률에 더 직접적인 영향을 주고 있습니다.`,
} as const;
